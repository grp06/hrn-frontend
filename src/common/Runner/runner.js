...
class Runner extends React.Component {
  ...
  onKeyDown(e) {
    ...
    switch (code) {
      case this.config.KEYCODE_JUMP:
        if (this.status !== STATUS.RUNNING && performance.now() - this.restartLock > 500) {
          this.restart()
        }
        this.tRex.jump()
        break
  ...
  update() {
    ...
    if (this.status === STATUS.RUNNING) {
      ...
      if (
        ...
        this.currentSpeed +=
          this.config.ACCELERATION * (this.accelerationTime / this.config.ACCELERATION_INTERVAL)
  ...
  checkCollision() {
    return this.groundManager.obstacleList.some((obstacle) => obstacle.isOverlap(this.tRex))
  }
